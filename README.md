...
- Store 브랜드 랭킹 API 구현
- Store 즐겨찾기 View 구현
- Home API, Store API에 이미지 관련 변경사항 적용
- Store 즐겨찾기 추가, 삭제, 조회 API 구현
- 회원탈퇴 API 구현 

#### 개발 중 이슈 
1. 즐겨찾기 버튼을 클릭했을 때, API연결을 해야하는데 버튼의 index를 알지 못해서 구현이 어려웠음 -> 버튼에 태그를 index 값으로 설정한 후, API 연결을 했음
2. 스토어의 쇼핑몰, 브랜드 랭킹을 보여줄 때, 즐겨찾기 버튼을 클릭하면 서로의 셀에도 그대로 적용됨 -> 둘의 Cell을 하나로 했던게 문제, 따로따로 Cell을 나눠서 구현하였음
